...
layui.define(['layer', 'laytpl', 'upload'], function (exports) {
    ...
    //接受消息
    LAYIM.prototype.getMessage = function (data, isVoice) {
        return getMessage(data, isVoice), this;
    };
    LAYIM.prototype.showQuestion = function (content) {
        var data = {
    ...
    //自身发送消息(为什么不调用sendMessage：im.js中默认sendMessage绑定向服务器发送消息)
    ...
    layui.data.content = function (content) {
        ...
        var html = function (end) {
            return new RegExp('\\n*\\[' + (end || '') + '(pre|a|span|div|p|table|thead|th|tbody|tr|td|ul|li|ol|li|dl|dt|dd|h2|h3|h4|h5)([\\s\\S]*?)\\]\\n*', 'g');
        };
...
//判断是否需要去后台查询数据
    ...
    //接受消息
    var messageNew = {}, getMessage = function (data, isVoice) {
        data = data || {};
        ...
        messageNew = JSON.parse(JSON.stringify(data));//此深拷贝方式在ie9-ie11中有时会出错
        // $.extend(true,messageNew,data);

        if (cache.base.voice && isVoice) {
            voice();
        }
    ...
    var chatListMore = function () {
        ...
        if (length >= MAX_ITEM) {
            ...
            var more = ul.prev('.layim-chat-system')
    ...
    'modules/layim/layim.css?v=3.54Pro'
    , 'skinlayimcss'
);